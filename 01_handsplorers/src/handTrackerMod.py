...
from src.config import recheck, keystrokes, distance_x, distance_y
...
class handDetector():
    ...
    def check_gesture(self, className):
        ...
        if className == 'thumbs down' and recheck:
            ...
            sleep(keystrokes['sleep'])
            recheck = True
        elif className in ('okay', 'stop', 'live long', 'fist', 'smile', 'peace', 'rock') and recheck: 
            recheck = False
            sleep(keystrokes['sleep'])
            recheck = True
    ...
    def mouse_control(self, x, y, frame):
        ...
        if self.results.multi_hand_landmarks:
            for handLandmarks in self.results.multi_hand_landmarks:
                mouse_point = handLandmarks.landmark[self.mpHands.HandLandmark.MIDDLE_FINGER_MCP]
                ...
                if mouse_point.x * x > quadrant_width and mouse_point.y * y < quadrant_height:
                    indexfingertip_x, indexfingertip_y = int(
                        indexfingertip.x * x), int(indexfingertip.y * y)
                    thumbfingertip_x, thumbfingertip_y = int(
                        thumbfingertip.x * x), int(thumbfingertip.y * y)

                    # Transformed coordinate system for mapping the cursor
                    cursor_x = int((mouse_point.x - 0.5) * 2 * win32api.GetSystemMetrics(0))
                    ...
                    if Distance_x < self.distance_x and Distance_y < self.distance_y:
                        ...
                        if self.mouse_down_state:
                            cv2.putText(frame, "CLICK DOWN", (10, 120), cv2.FONT_HERSHEY_SIMPLEX,
